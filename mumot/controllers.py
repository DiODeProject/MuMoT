    ...
    def _addSpecificWidgets(self, MAParams, continuousReplot=False):
        ...
        if not MAParams['netType'][-1]:
            netDropdown = widgets.Dropdown(
                options=[('Full graph', models.NetworkType.FULLY_CONNECTED),
                         ('Erdos-Renyi', models.NetworkType.ERSOS_RENYI),
                         ('Barabasi-Albert', models.NetworkType.BARABASI_ALBERT),
                         # @todo: add network topology generated by random points in space
                         ('Moving particles', models.NetworkType.DYNAMIC)
                         ],
                description='Network topology:',
                value=models._decodeNetworkTypeFromString(MAParams['netType'][0]),
                style={'description_width': 'initial'},
                disabled=False
            )
        ...
        if not MAParams['showTrace'][-1]:
            widget = widgets.Checkbox(
                ...
                disabled=False  # not (self._widgetsExtraParams['netType'].value == models.NetworkType.DYNAMIC)
            )
            ...
            for key in kwargs.keys() | initWidgets.keys():
                ...
                if optionValues[-1] is True:
                    ...
                    if key == 'netType':  # netType is special
                        optionValues[0] = models._decodeNetworkTypeFromString(optionValues[0])  # @todo: if only netType (and not netParam) is specified, then multicotroller won't work...
